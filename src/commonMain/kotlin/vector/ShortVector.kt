...
// @formatter:off
inline class ShortVector(val data: ShortArray) {
    /// Index Constructor
    constructor(shape: Int, init: (Int) -> Short) : this(ShortArray(shape) { init(it) })
    constructor(shape: Int) : this(ShortArray(shape))
    ...
    operator fun set(i: Int, value: Short) = run { data[min(i, data.size + i)] = value }
    operator fun get(i: IntProgression) = ShortVector((i.last - i.first + 1) / i.step).apply {
        i.forEachIndexed { i, v -> this@apply[i] = this@ShortVector[v] }
    }
    
    /// Unary operations
    operator fun unaryPlus()  = IntVector(data.size) { +data[it] }
    operator fun unaryMinus() = IntVector(data.size) { -data[it] }
    operator fun inc()        = ShortVector(data.size) { data[it]++ }
    operator fun dec()        = ShortVector(data.size) { data[it]-- }
    ...
    operator fun plusAssign(b: Short) = data.forEachIndexed { i, _ -> data[i] = (data[i] + b).toShort() }
    operator fun minusAssign(b: Short) = data.forEachIndexed { i, _ -> data[i] = (data[i] - b).toShort() }
    operator fun timesAssign(b: Short) = data.forEachIndexed { i, _ -> data[i] = (data[i] * b).toShort() }
    operator fun divAssign(b: Short) = data.forEachIndexed { i, _ -> data[i] = (data[i] / b).toShort() }
    operator fun remAssign(b: Short) = data.forEachIndexed { i, _ -> data[i] = (data[i] % b).toShort() }
    
    /// Assign Vector Operations
    operator fun plusAssign(b: ShortVector) = data.forEachIndexed { i, _ -> data[i] = (data[i] + b[i]).toShort() }
    operator fun minusAssign(b: ShortVector) = data.forEachIndexed { i, _ -> data[i] = (data[i] - b[i]).toShort() }
    operator fun timesAssign(b: ShortVector) = data.forEachIndexed { i, _ -> data[i] = (data[i] * b[i]).toShort() }
    operator fun divAssign(b: ShortVector) = data.forEachIndexed { i, _ -> data[i] = (data[i] / b[i]).toShort() }
    operator fun remAssign(b: ShortVector) = data.forEachIndexed { i, _ -> data[i] = (data[i] % b[i]).toShort() }
